...
test("Login using ENV Vars & list devices", async () => {
  ...
  const device = devices.find((x) => x.alias === "Drinks Lamp");
  const plug = cloudApi.getTapoDevice(device);
...
xtest("Get device info", async () => {
  ...
  // const alarmTones = await device.playAlarm(AlarmTone.DoorbellRing8, AlarmVolume.high);
  // console.log(alarmTones)
  const eventLogs = await device.getEventLogs(
    "802E9E6F329DEB5C8609211DD020F75A24C185F0",
  );
